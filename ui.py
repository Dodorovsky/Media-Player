...
def load_images(self):
    ...
    self.prev_button.config(image=self.previous_img)
...
def setup_ui(self):
    ...
    self.logo_listbox = tk.Label(self.listbox, bg="black")
    ...
    self.current_time_label.grid(row=3, column=0, padx=2, pady=(0, 0), sticky="w")
    ...
    self.total_time_label.grid(row=3, column=5, padx=2,pady=(0, 0))
    ...
    self.playlist_button = tk.Menubutton(
        self.left_frame,
        relief="groove",
        bg="#BC853D",
        fg="black",
    ...
    self.load_label_frame = tk.Label(
        self.left_frame,
        text=' LOAD_MEDIA',
        font=("Terminal", 8),
        bg="#2C2929",
    ...
    self.next_button = tk.Button(self.sub_frame_2, command=self.play_next)
    ...
    self.volume_label_frame = tk.Label(
        self.right_frame,
        text='[.....VOLUME......]',
        font=("Terminal", 8),
        bg="#2C2929",
    ...
    self.fullscreen_button = tk.Button(
        ...
        fg="#E0D2D2",
        font=("Terminal", 7),
        command=self.enter_fullscreen_video
    )
    self.fullscreen_button.grid(pady=5)
    self.root.bind("<Escape>", lambda e: self.exit_fullscreen_video())

    self.mp6_label_left = tk.Label(self.vu_frame_left, bg="#2C2929")
    ...
    for i, (name, url) in enumerate(self.radios.items()):
        btn = tk.Button(
            ...
            text=name,
            font=("Terminal", 7),
            command=lambda n=name: self.play_radio(n),
            bg="#191818",
            fg="#E0D2D2"
        )
    ...
    self.radios_labels.grid(padx=(0), pady=(0, 5), row=0)

    self.mp6_label_right = tk.Label(self.vu_frame_right, bg="#2C2929")
    self.mp6_label_right.grid(padx=(0), pady=(0), sticky="s")

    self.eq_line = tk.Label(
        self.main_frame,
        bg="#2C2929",
    ...
    frequencies = ["80Hz", "250Hz", "1kHz", "4kHz", "10kHz"]
    self.eq_sliders = []
    ...
    for i, freq in enumerate(frequencies):
        ...
        label = tk.Label(self.eq_light_frame, bg="#2C2929")
    ...
    # Programar la carga de imágenes cuando Tkinter ya está listo
    self.root.after_idle(lambda: load_images(self))
